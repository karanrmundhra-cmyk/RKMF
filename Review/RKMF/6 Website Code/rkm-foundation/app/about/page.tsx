...
function PersonCard({ p }: { p: { name: string; role: string; img: string | null } }) {
  const initials = p.name.split(" ").map((w) => w[0]).slice(0, 2).join("");
  return (
    <div className="card p-5 text-center">
      {p.img ? (
        <img src={p.img} alt={p.name} className="mx-auto h-24 w-24 rounded-full object-cover" loading="lazy" />
      ) : (
        <div className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-copper/15 text-xl font-bold text-copper-dark">{initials}</div>
      )}
      <div className="mt-4 font-semibold">{p.name}</div>
      <div className="mt-0.5 text-xs uppercase tracking-wide text-ink/60">{p.role}</div>
    </div>
  );
}
...
export default function AboutPage() {
  return (
    <>
      <section className="bg-snow pb-20 pt-32 sm:pb-24 sm:pt-40">
        <div className="container-c grid items-center gap-12 lg:grid-cols-2">
          <Reveal>
            <p className="eyebrow">About RKM Foundation</p>
            <h1 className="h-display mt-4 text-4xl sm:text-5xl lg:text-6xl">We&apos;re a family that started helping animals, and kept at it.</h1>
          ...
          <Reveal delay={120}>
            <div className="img-hover aspect-[4/3]">
              <img src="/images/pillars/my-animals.jpeg" alt="A rescued animal at the RKM shelter" className="object-cover" />
            </div>
          </Reveal>
        </div>
      </section>
      ...
      <section className="py-24 sm:py-32">
        <div className="container-c max-w-3xl">
          <Reveal>
            <p className="eyebrow text-center">How It Started</p>
            <h2 className="h-display mt-4 text-center text-3xl sm:text-4xl lg:text-5xl">It started with our dog, Tobler.</h2>
            ...
            <div className="card-static mt-10 p-6 sm:p-7">
      ...
      <section className="bg-snow py-24 sm:py-32">
        <div className="container-c">
          <Reveal>
            ...
            <h2 className="h-display mt-4 text-center text-3xl sm:text-4xl lg:text-5xl">What we hold ourselves to</h2>
          </Reveal>
      ...
      <section className="py-24 sm:py-32">
        <div className="container-c">
          <Reveal>
            <p className="eyebrow text-center">Be Part of the Mission</p>
            <h2 className="h-display mt-4 text-center text-3xl sm:text-4xl lg:text-5xl">Five ways to belong</h2>
            ...
            {ROLES.map((r, i) => (
              <Reveal key={r.title} delay={i * 70}>
                <div className="card flex h-full flex-col p-6">
                  ...
                  <p className="mt-1 text-xs font-medium uppercase tracking-wider text-copper-dark">{r.virtues}</p>
                  <p className="mt-3 flex-1 text-sm leading-relaxed text-ink/70">{r.desc}</p>
                  <Link href={r.href} className="link-secondary mt-4 text-sm">{r.cta} →</Link>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section className="bg-snow py-24 sm:py-32">
        <div className="container-c">
          <Reveal>
            ...
            <h2 className="h-display mt-4 text-center text-3xl sm:text-4xl lg:text-5xl">A community that cares</h2>
          </Reveal>
          <Reveal>
            <h3 className="mt-12 text-center text-sm font-bold uppercase tracking-wider text-ink/60">Trustees</h3>
            <div className="mx-auto mt-5 grid max-w-3xl gap-5 sm:grid-cols-3">
              {TEAM.trustees.map((p, i) => <Reveal key={p.name} delay={i * 60}><PersonCard p={p} /></Reveal>)}
            </div>
          </Reveal>
          <Reveal>
            <h3 className="mt-12 text-center text-sm font-bold uppercase tracking-wider text-ink/60">Core Management</h3>
            <div className="mt-5 grid gap-5 sm:grid-cols-3 lg:grid-cols-6">
              {TEAM.core.map((p, i) => <Reveal key={p.name} delay={i * 60}><PersonCard p={p} /></Reveal>)}
            </div>
          </Reveal>
          <Reveal>
            <h3 className="mt-12 text-center text-sm font-bold uppercase tracking-wider text-ink/60">Lead Coordinators</h3>
            <div className="mx-auto mt-5 grid max-w-md gap-5 sm:grid-cols-2">
              {TEAM.coordinators.map((p, i) => <Reveal key={p.name} delay={i * 60}><PersonCard p={p} /></Reveal>)}
            </div>
          </Reveal>
        </div>
      </section>
